...
In **Approach #3**, Zac  applied a clustering approach to transactional data, focusing on the relationship between gallons and cases ordered versus what was actually delivered. He created a new column that captures fulfillment and returns, providing a clearer view of how orders are being met.

In **Approach #4**, Georgia chose a forecasting approach to predict customer-level future orders with the goal of identifying trends in customer behavior and potential growth patterns. Due to the limited data and time constraints, I developed a base-level forecast model that incorporates features like lag values, rolling averages, and growth rates to capture recent order patterns and momentum. While this model offers some insights, there’s opportunity for further improvement by adding advanced features, improving segmentation, and exploring additional data sources. 
...
customer_neighbor_fields <- as.data.table(data.table::fread("data/derived/customer_neighbor_fields.csv"))
...
```{r include = FALSE}
swire_data_full <- readRDS('data/derived/swire_data_full.Rds')
```
...
# write.csv(CLARA_LMP, "data/derived/CLARA_LMP.csv", row.names = FALSE)
...
![](imgs/nearest-customers-diagram.png)
...
```{r eval = FALSE}
if(file.exists('data/derived/cust_dist.rda')) {
    load('data/derived/cust_dist.rda')  # Load precomputed customer distance data if available
} else {
    kansas.data <- swire_cust_grp |> filter(state == 'Kansas')  # Filter for customers within each state
    kansas.dist <- distm(kansas.data[,c("lon", "lat")], fun = distHaversine)  # Compute pairwise Haversine distances
    ...
    cust_dist <- list(
        "louisiana" = list(
            "dist" = louisiana.dist,  # Distance matrix
            "cust" = louisiana.data$customer_number,  # Customer IDs
            "group" = louisiana.data$primary_group_number  # Group IDs
        ), 
        "kansas" = list(
            "dist" = kansas.dist, 
    ...
    save(cust_dist, file = "data/cust_dist.rda")  # Save list for future reuse
}
```

```{r eval = FALSE}
# Replace diagonals in each matrix
...
get_top5 <- function(row) {
    idx <- order(setdiff(row, c(0)))[1:5]  # Get indices of 5 smallest non-zero values
    values <- row[idx] / 1000  # Scale selected values (e.g., convert meters to kilometers)
    list(indices = idx, values = values)  # Return both positions and scaled values
}
```

```{r eval = FALSE}
# Initialize empty results data frame with correct types
dist_df <- data.frame(matrix(NA, nrow = 0, ncol = 4)) |>
    mutate(
        X1 = as.character(X1),  # Placeholder for customer_number
        X2 = as.character(X2),  # Placeholder for primary_group_number
        X3 = as.integer(X3),    # Placeholder for indices
        X4 = as.numeric(X4)     # Placeholder for distances
    )
colnames(dist_df) <- c("customer_number", "primary_group_number", "indices", "distances")

# Iterate through each state’s distance list
for(state in names(cust_dist)) {
    results <- apply(cust_dist[[state]]$dist, 1, get_top5)  # Get top 5 closest neighbors for each customer

    results_tbl <- tibble(
        customer_number = as.character(cust_dist[[state]]$cust),  # Focal customer ID
        primary_group_number = as.character(cust_dist[[state]]$group),  # Customer's group
        indices = lapply(results, function(x) x$indices),  # List of nearest neighbor indices
        distances = lapply(results, function(x) x$values)  # List of corresponding distances
    ) |>
        unnest_longer(c("indices", "distances")) |>  # Expand each row to one row per neighbor
        inner_join(
            tibble(
                neighbor_number = as.character(cust_dist[[state]]$cust)  # Lookup table for matching index to customer ID
            ) |> rowid_to_column(),  # Add index for join (1-based row index)
            join_by(indices == rowid)  # Match neighbor index to actual customer ID
        )

    dist_df <- bind_rows(dist_df, results_tbl)  # Append results to final data frame
}
```
...
customer_neighbors <- dist_df |>
    as_tibble() |>
    inner_join(
        swire_cust_clean,  # Join neighbor details from customer-level data
        join_by(neighbor_number == customer_number)
    ) |>
    mutate(
        same_group_flg = primary_group_number.x == primary_group_number.y  # Flag if neighbor is in same group
    ) |>
    group_by(customer_number) |>
    summarise(
        neighbor_avg_dist_km = mean(distances),  # Avg distance to neighbors (in km)
        neighbor_primary_group_count = sum(same_group_flg, na.rm = TRUE),  # Count of neighbors in same group
        neighbor_local_market_partners = sum(local_market_partner),  # Number of neighbors that are LMP
        neighbor_avg_return_freq = (mean(return_frequency_2023) + mean(return_frequency_2024)) / 2,  # Avg return frequency across years
...
customer_neighbor_fields <- 
    customer_neighbors |>
    mutate(
        neighbor_avg_dist_km = ifelse(
            is.infinite(neighbor_avg_dist_km), 
            max(ifelse(!is.infinite(neighbor_avg_dist_km), neighbor_avg_dist_km, NA), na.rm = TRUE),  # Replace Inf with max finite distance
            neighbor_avg_dist_km
        )
    ) |>
    replace_na(list(
        neighbor_avg_dist_km = median(customer_neighbors$neighbor_avg_dist_km, na.rm = TRUE),  # Replace NA with median
...
```{r}
swire_numeric <- swire_cust_enriched |> 
    select(where(is.numeric))  # Select all numeric columns for modeling or analysis

swire_non_numeric <- swire_cust_enriched |> 
    select(!where(is.numeric)) |>  # Select all non-numeric columns
    select(-c(customer_number, primary_group_number, sub_trade_channel)) |>  # Drop ID fields and sub_trade_channel
    mutate(
        across(everything(), ~factor(make.names(.)))  # Standardize values (safe for model.matrix) and convert to factors
    )
```
...
swire_cust_combined <-
    swire_numeric |> 
    bind_cols(data_dmy) |>  # Combine numeric fields with dummy-coded categorical variables
    select(-c(trade_channel.PHARMACY.RETAILER))  # Drop a specific one-hot encoded column, likely due to collinearity
```
...
cluster_kmeans <- function(k, data) {
    fit <- kmeans(data, k)  # Run k-means clustering with k clusters
    vals <- glance(fit)  # Get summary metrics
    return(vals$tot.withinss)  # Return total within-cluster sum of squares
}

cluster_hclust <- function(k, data) {
    # Initialize hierarchical clustering model with complete linkage
    model <- hier_clust(num_clusters = k, linkage_method = "complete")

    # Fit the model to the data
    fit <- model |> fit(~., data = as.data.frame(data))

    # Compute and return the total within-cluster sum of squares
    wss <- fit |>
        sse_within() |>  # Compute WSS for each cluster
        select(wss) |>  # Extract WSS column
...
```{r eval = FALSE}
set.seed(814)  # Ensure reproducibility for sampling and clustering

swire_pca_80 <- swire_pca_results[,1:33]  # Select first 33 PCs explaining ~80% variance
swire_pca_80_sampl <- swire_pca_80 |> sample_n(size = 5000)  # Randomly sample 5,000 rows for faster clustering

clustering_grid <- data.frame(
    clusters = 2:8  # Define range of cluster counts to evaluate
)
```
...
show_cluster_results <- function(results, title) {
    results |>
        pivot_longer(cols = -clusters) |>  # Reshape to long format for ggplot
        unnest(value) |>  # Expand list-columns (if results are stored that way)
        ggplot(aes(
            factor(clusters),  # X-axis: number of clusters (as a factor for discrete spacing)
            as.numeric(value),  # Y-axis: WSS values
            group = name  # Group by clustering method
        )) +
        geom_line(
            color = swire_colors$red,  # Dotted red line for visual reference
            linetype = "dotted", 
            size = 2
        ) +
        geom_smooth(
            ...
            se = FALSE  # Add smoothed trend line, no confidence band
        ) +
        labs(
            title = title,
        ...
        facet_wrap(~name, ncol = 1, scales = "free") +  # Separate plots for each method
        theme_swire()
}
```
...
clustering_grid_results_sampl_80 <-
    clustering_grid |>
    mutate(
        kmeans = map(clusters, ~ cluster_kmeans(.x, swire_pca_80_sampl)),  # Compute WSS for k-means at each k
        hclust = map(clusters, ~ cluster_hclust(.x, swire_pca_80_sampl))   # Compute WSS for hierarchical clustering at each k
    )
```
...
![](imgs/cluster_results_comparison.png)
...
clustering_grid_results_80 <-
    clustering_grid |>
    mutate(
        kmeans = map(clusters, ~ cluster_kmeans(.x, swire_pca_80)),  # Run k-means clustering on full PCA dataset
        hclust = map(clusters, ~ cluster_hclust(.x, swire_pca_80))   # Run hierarchical clustering on full PCA dataset
    )
```
...
```{r eval = FALSE}
# Derive fitted cluster assignments from hierarchical approach
...
```{r include = FALSE}
# saveRDS(cluster_assignments, file = "data/derived/cluster_assignments.Rds")

cluster_assignments <- readRDS("data/derived/cluster_assignments.Rds")
```
...
transactional_df <- transactions %>%
  clean_names() %>%  # Standardize column names to snake_case
  mutate(
    transaction_date = ymd(transaction_date),  # Parse transaction dates
    customer_number = as.character(customer_number)  # Ensure consistent ID type
  ) %>%
  group_by(customer_number) %>%
  summarise(
    total_ordered_cases = sum(ordered_cases, na.rm = TRUE),
    total_delivered_cases = sum(delivered_cases, na.rm = TRUE),
    net_delivered_cases = sum(ifelse(delivered_cases < 0, 0, delivered_cases), na.rm = TRUE),  # Set negatives to 0
    fulfillment_rate = ifelse(total_ordered_cases == 0, NA, net_delivered_cases / total_ordered_cases),  # Prevent division by 0
    total_returned_cases = sum(abs(delivered_cases[delivered_cases < 0]), na.rm = TRUE),  # Sum of negative case volumes
    total_ordered_gallons = sum(ordered_gallons, na.rm = TRUE),
    total_delivered_gallons = sum(delivered_gallons, na.rm = TRUE),
    net_delivered_gallons = sum(ifelse(delivered_gallons < 0, 0, delivered_gallons), na.rm = TRUE),  # Remove negative gallon values
  ...
  filter(!is.na(fulfillment_rate) & !is.na(fulfillment_rate_gallons))  # Keep only customers with valid data
...
clustering_data <- transactional_df %>%
  select(
    total_ordered_cases, total_delivered_cases, fulfillment_rate,
    total_ordered_gallons, total_delivered_gallons, fulfillment_rate_gallons
  ) %>%
  scale()  # Standardize for k-means

kmeans_model <- kmeans(clustering_data, centers = 4, nstart = 10)  # Run k-means with 4 clusters
transactional_df$cluster <- kmeans_model$cluster  # Assign cluster labels to original data
```
...
While the clustered fulfillment model offers insights into high-performing and high-potential customers, identifying individual customers would provide additional value for salespeople and other points of contact who have direct relationships with them. Their personal experience could help interpret the data more effectively. Additionally, incorporating features that classify customers by business type could enhance clustering, offering more precise insights into fulfillment rates across different industries.
...
neighbors <- customer_neighbor_fields
...
By leveraging data-driven modeling techniques, we enhance Swire Coca-Cola’s ability to segment customers and predict ordering behavior. These insights enable more efficient resource allocation, reduce costs, and identify growth opportunities. Moving forward, continuous refinement of models and integration of additional data sources will further optimize decision-making and operational strategies.

## Division of Work

This group modeling assignment was made possible through the collaborative efforts of all team members. Together, we gathered and cleaned transactional, customer, and delivery cost data, ensuring accuracy and consistency. We engineered key features to strengthen clustering and predictive modeling, utilizing various techniques to enhance customer segmentation and forecasting. Through our combined efforts, we refined data-driven strategies to optimize Swire Coca-Cola’s distribution model.
