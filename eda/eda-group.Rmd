...
```{r echo = FALSE}
# Create a table using `gt` to summarize dataset metadata
gt(tibble(
  file = c(
  ...
  # Center-align specific columns for visual consistency
  cols_align(
    align = "center", 
    columns = c("format", "colnum", "rownum")  # Align file format and dimensions
  ) |>
  
  # Format numeric columns to show no decimal places
  fmt_number(
    columns = c("colnum", "rownum"),  # Apply formatting only to numeric summary columns
    decimals = 0
  ) |>
  
  # Customize table styling with a background color for column labels
  tab_options(
    column_labels.background.color = swire_colors$red  # Apply custom color from palette
  )
```
