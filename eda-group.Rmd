...
Below we see a graph showing that higher order types (from cases) come from my coke legacy. This could either just be the fact that most sales come from my coke legacy and sales rep or it could provide further information on possible predictions for high growth potential. When we look at just the customers with < 400 volume, call center order type becomes the most common and sales rep falls to third, suggesting that order type could be a valuable characteristic for identifying high growth potential. 
...
ggplot(order_type_summary, aes(x = reorder(order_type, -total_cases), y = total_cases, fill = order_type)) +
...
In the graphs below, dining and goods are the main cold drink channels for customers with a volume of less than 400 gallons, while bulk trade and dining dominate for all customers. This could suggest the need for a separate 'dining' cold drink channel analysis to compare characteristics between the two thresholds.
